...
export class AdminController {
  ...
  async showAdminPanel(ctx: BotContext): Promise<void> {
    ...
    await safeEditMessage(ctx, MESSAGES.ADMIN_PANEL, Markup.inlineKeyboard([[Markup.button.callback('📋 Встречи', 'admin_events'), Markup.button.callback(BUTTONS.CREATE_EVENT, 'create_event')], [Markup.button.callback('💳 Реквизиты для оплаты', 'payment_details')], [Markup.button.callback('ℹ️ О клубе', 'info')], [Markup.button.callback(BUTTONS.MAIN_MENU, 'main_menu')]]))
  }
  ...
  async confirmPayment(ctx: BotContext, eventId: number, userId: number): Promise<void> {
    ...
    try {
      ...
      await safeEditMessage(ctx, (ctx.callbackQuery?.message as any)?.text + '\n\n✅ Оплата подтверждена', Markup.inlineKeyboard([]))
    } catch (error) {
