...
export class PaymentDetailsController {
  ...
  async showPaymentDetailsList(ctx: BotContext): Promise<void> {
    ...
    try {
      ...
      await safeEditMessage(ctx, message, Markup.inlineKeyboard([[Markup.button.callback('➕ Добавить реквизиты', 'add_payment_details')], [Markup.button.callback('✏️ Редактировать реквизиты', 'edit_payment_details')], [Markup.button.callback('◀️ Назад', 'admin')]]))
    } catch (error) {
  ...
  async showEditPaymentDetailsList(ctx: BotContext): Promise<void> {
    ...
    try {
      ...
      await safeEditMessage(ctx, 'Выберите реквизиты для редактирования или удаления:', Markup.inlineKeyboard(buttons))
    } catch (error) {
  ...
  async confirmDeletePaymentDetails(ctx: BotContext, detailsId: number): Promise<void> {
    ...
    try {
      ...
      await safeEditMessage(ctx, `Вы уверены, что хотите удалить реквизиты "${details.title}"?`, Markup.inlineKeyboard([[Markup.button.callback('✅ Да', `confirm_delete_payment_details_${detailsId}`)], [Markup.button.callback('❌ Нет', 'edit_payment_details')]]))
    } catch (error) {
